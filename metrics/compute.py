...
from ._utils import CellContext
...
# Registry of building/street metrics: (name, function)
_BUILDING_METRICS = [
    ("courtyard_area", courtyard_area_metrics),
    ("floor_area", floor_area_metrics),
    ("longest_axis_length", longest_axis_length_metrics),
    ("perimeter_wall", perimeter_wall_metrics),
    ("volume", volume_metrics),
    ("circular_compactness", circular_compactness_metrics),
    ("square_compactness", square_compactness_metrics),
    ("convexity", convexity_metrics),
    ("courtyard_index", courtyard_index_metrics),
    ("rectangularity", rectangularity_metrics),
    ("shape_index", shape_index_metrics),
    ("corners", corners_metrics),
    ("squareness", squareness_metrics),
    ("equivalent_rectangular_index", equivalent_rectangular_index_metrics),
    ("elongation", elongation_metrics),
    ("facade_ratio", facade_ratio_metrics),
    ("fractal_dimension", fractal_dimension_metrics),
    ("form_factor", form_factor_metrics),
    ("compactness_weighted_axis", compactness_weighted_axis_metrics),
    ("centroid_corner_distance", centroid_corner_distance_metrics),
    ("orientation", orientation_metrics),
    ("shared_walls", shared_walls_metrics),
    ("alignment", alignment_metrics),
    ("neighbor_distance", neighbor_distance_metrics),
    ("mean_interbuilding_distance", mean_interbuilding_distance_metrics),
    ("building_adjacency", building_adjacency_metrics),
    ("neighbors", neighbors_metrics),
    ("cell_alignment", cell_alignment_metrics),
    ("street_alignment", street_alignment_metrics),
    ("courtyards", courtyards_metrics),
    ("street_profile", street_profile_metrics),
    ("nearest_street_distance", nearest_street_distance_metrics),
]
...
ALL_METRIC_NAMES: list[str] = [name for name, _ in _BUILDING_METRICS] + _CONNECTIVITY_METRIC_NAMES
...
def compute_all_metrics(
    ...
    Args:
        ctx: CellContext with pre-projected focal and neighbourhood data.
    ...
    for name, fn in _BUILDING_METRICS:
        ...
        if cell_gdf is None:
            ...
            result = _run_metric(cell_gdf, fn, name, ctx, merge=merge, quiet=quiet)
            if return_dict:
    ...
    if any_connectivity:
        ...
        t0 = time.perf_counter()
        connectivity_results = compute_connectivity_metrics_by_name(ctx)
        if not quiet:
